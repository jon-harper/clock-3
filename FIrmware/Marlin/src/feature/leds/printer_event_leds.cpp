...
PrinterEventLEDs printerEventLEDs;
  ...
  inline void pel_set_rgb(const uint8_t r, const uint8_t g, const uint8_t b OPTARG(HAS_WHITE_LED, const uint8_t w=0)) {
    LEDColor new_color(r, g, b OPTARG(HAS_WHITE_LED, w) OPTARG(NEOPIXEL_LED, neo.brightness()));
    leds.set_color( new_color, TERN(NEOPIXEL_IS_SEQUENTIAL, true, false));
  #if defined(NEOPIXEL2_SEPARATE) && defined(PRINTER_EVENT_LEDS_UPDATE_ALL)
    leds2.set_color(new_color);
  #endif
  }
